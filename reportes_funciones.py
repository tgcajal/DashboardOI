...
def indicadores_montos(df):
    ...
    montos = pd.DataFrame({'Monto':['Capital promedio otorgado', 'Plazo promedio en cuotas'],
                       'Valores':['$'+str(round(df.sort_values(by='num_cuota').drop_duplicates(subset=['id_credito'],keep='first')['valor_financiamiento'].mean(),2)), round(df.drop_duplicates(subset=['id_credito'])['numero_periodos'].mean()/2)]})
